...
const { upsertRoomFromPayload } = require('../services/room-sync.service');

const EXCHANGE = 'hotel_events';
const DLX = 'hotel_events_dlx';
const QUEUE = 'q.search_service';
...
async function handleMessage(msg) {
  ...
  try {
    if (routingKey === 'room.created' || routingKey === 'room.updated') {
      await upsertRoomFromPayload(parsed.payload || {});
    }
    channel.ack(msg);
  } catch (err) {
    logger.error('search-service consumer error', { routingKey, message: err.message, stack: err.stack });
    channel.nack(msg, false, false);
  }
}
    ...
    try {
      ...
      await ch.assertQueue(QUEUE, {
        ...
        arguments: {
          ...
          'x-dead-letter-routing-key': 'dead.search_service',
        },
      });
      await ch.bindQueue(QUEUE, EXCHANGE, 'room.created');
      await ch.bindQueue(QUEUE, EXCHANGE, 'room.updated');
