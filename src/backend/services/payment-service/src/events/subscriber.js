...
const { handleBookingCancelled } = require('../services/booking-cancelled.service');

const EXCHANGE = 'hotel_events';
const DLX = 'hotel_events_dlx';
const QUEUE = 'q.payment_service';
...
async function handleMessage(msg) {
  ...
  try {
    if (routingKey === 'booking.cancelled') {
      await handleBookingCancelled(parsed.payload || {});
    }
    channel.ack(msg);
  } catch (err) {
    logger.error('payment-service consumer error', { routingKey, message: err.message, stack: err.stack });
    channel.nack(msg, false, false);
  }
}
    ...
    try {
      ...
      await ch.assertQueue(QUEUE, {
        ...
        arguments: {
          ...
          'x-dead-letter-routing-key': 'dead.payment_service',
        },
      });
